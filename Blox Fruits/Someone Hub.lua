...
return game:GetService("Workspace")['_WorldOrigin'].Locations:FindFirstChild(Island)
  ...
  function GetFruitInventory(v222)
...
local function Tween(targetPosition)
    ...
    while (humanoidRootPart.Position - targetPosition).magnitude > stopDistance do
        ...
        local speed
        if distance <= 25 then
            speed = math.max(minSpeed, getgenv().TweenSpeed * (distance / 25))
        else
...
local function BoatTween(targetPosition)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoidRootPart = character:WaitForChild("HumanoidRootPart")
    ...
    while (humanoidRootPart.Position - targetPosition).magnitude > stopDistance do
        if humanoid.Sit ~= true then break end -- Se não estiver sentado, sai do loop

        local distance = (humanoidRootPart.Position - targetPosition).magnitude
        local speed = distance <= 50 and math.max(minSpeed, 350 * (distance / 50)) or 350
        local direction = (targetPosition - humanoidRootPart.Position).unit
...
local function BoatTween(targetPosition)
    ...
    while (humanoidRootPart.Position - targetPosition).magnitude > stopDistance do
        ...
        local distance = (humanoidRootPart.Position - targetPosition).magnitude
        local speed = distance <= 25 and math.max(minSpeed, 350 * (distance / 25)) or 350
        local direction = (targetPosition - humanoidRootPart.Position).unit
        ...
        if _G.BringFruitBF then
            pcall(function()
                ...
                if string.find(v804.Name, "Fruit") then
                        ...
                        v.Handle.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                        wait()
                       v.Handle.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                        wait()
                        v.Handle.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
...
spawn(function()
    while wait() do
        if _G.AutoBuyChip and not _G.AutoStoreSsFruit then
            pcall(function()
    ...
    while true do
        ...
        pcall(function()
            if _G.AutoStartRaid then
                ...
                local gui = game.Players.LocalPlayer.PlayerGui.Main.TopHUDList
                if not gui.RaidTimer.Visible then
                    if not workspace['_WorldOrigin'].Locations:FindFirstChild("Island 1") and Backpack("Special Microchip") then
                        if Sea2 then
                            Tween(Vector3.new(-6438, 250, -4501))
                            repeat task.wait() until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - Vector3.new(-6438, 250, -4501)).Magnitude < 5
                            FireRemote("SetSpawnPoint")
                            fireclickdetector(workspace.Map.CircleIsland.RaidSummon2.Button.Main.ClickDetector)
                        elseif Sea3 then
                            Tween(Vector3.new(-5017, 314, -2823))
                            repeat task.wait() until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - Vector3.new(-5017, 314, -2823)).Magnitude < 5
                            FireRemote("SetSpawnPoint")
        ...
        if AutoNextIsland then
            pcall(function()
                ...
                if root then
                    ...
                    local distance1 = (position - Vector3.new(-6438, 250, -4501)).Magnitude
                    local distance2 = (position - Vector3.new(-5017, 314, -2823)).Magnitude

                    if distance1 < 1 or distance2 < 1 then
                        v357 = {}
                    end

                    if locations:FindFirstChild("Island 1") and gui.RaidTimer.Visible then
                        _G.AutoFarmNearest = true
                    end

                    for i = 2, 5 do
                        local island = "Island " .. i
                        if locations:FindFirstChild(island) and not v357[island] then
                            Tween(locations[island].Position)
                            v357[island] = true
...
spawn(function()
    while wait() do
        pcall(function()
            ...
            if not Backpack("Rocket") or not Backpack("Spin") or not Backpack("Blade") or not Backpack("Spring") or not Backpack("Spike") or not Backpack("Smoke") or not Backpack("Flame") or not Backpack("Ice") or not Backpack("Sand") or not Backpack("Dark") or not Backpack("Light") or not Backpack("Diamond") or not Backpack("Magma")  or not Backpack("Rubber") or not Backpack("Ghost") then
                    LoadFruit("Rocket-Rocket")
                LoadFruit("Spin-Spin")
                    LoadFruit("Blade-Blade")
                    LoadFruit("Spring-Spring")
                    LoadFruit("Bomb-Bomb")
                    LoadFruit("Spike-Spike")
                    LoadFruit("Smoke-Smoke")
                    LoadFruit("Flame-Flame")
                    LoadFruit("Ice-Ice")
                    LoadFruit("Sand-Sand")
                    LoadFruit("Dark-Dark")
                    LoadFruit("Light-Light")
                    LoadFruit("Diamond-Diamond")
                    LoadFruit("Rubber-Rubber")
                    LoadFruit("Ghost-Ghost")
                    LoadFruit("Magma-Magma")
                end
            end
        end)
    end
...
Teleport:AddButton({
   ...
   if Sea1 then      -- Sea 1 Teleports
      ...
      elseif Island == "Frozen Village" then
         Tween(Vector3.new( 1378, 106, -1393))
   ...
   elseif Sea3 then
      if Island == "Mansion" then
         Tween(Vector3.new( -12549, 337, -7536))
...
local KILabel = Islands:AddParagraph({Title = "Kitsune Island : not spawn"})
  task.spawn(function()
...
local plrPP = game.Players.LocalPlayer.Character.HumanoidRootPart
          if plrPP then
            Distance = tostring(math.floor((plrPP.Position - Map.KitsuneIsland.WorldPivot.Position).Magnitude / 6))
          end
        end
      end
    end)
...
local PILabel = Islands:AddParagraph({Title = "Prehistoric Island : not spawn"})
task.spawn(function()
  local Map = workspace:WaitForChild("Map", 9e9)
  task.spawn(function()
    while true do
...
local plrPP = game.Players.LocalPlayer.Character.HumanoidRootPart
        if plrPP then
          Distance = tostring(math.floor((plrPP.Position - Map.PrehistoricIsland.WorldPivot.Position).Magnitude / 6))
        end
      end
    end
  end)
  ...
  while true do
    task.wait()
    if Map:FindFirstChild("PrehistoricIsland") then
      PILabel:SetTitle("Prehistoric Island : Spawned | Distance : " .. Distance)
    else
      PILabel:SetTitle("Prehistoric Island : not Spawn")
    end
  end
end)

local MILabel = Islands:AddParagraph({Title = "Mirage Island : not spawn"})
task.spawn(function()
  local Map = workspace:WaitForChild("Map", 9e9)
  task.spawn(function()
    while true do
      ...
      if Map:FindFirstChild("MysticIsland") then
        local plrPP = game.Players.LocalPlayer.Character.HumanoidRootPart
        if plrPP then
          Distance = tostring(math.floor((plrPP.Position - Map.MysticIsland.WorldPivot.Position).Magnitude / 6))
        end
      end
    end
  end)
  ...
  while true do
    task.wait()
    if Map:FindFirstChild("MysticIsland") then
      MILabel:SetTitle("Mirage Island : Spawned | Distance : " .. Distance)
    else
      MILabel:SetTitle("Mirage Island : not Spawn")
    end
  end
end)

local FDLabel = Islands:AddParagraph({Title = "Frozen Dimension : not spawn"})
task.spawn(function()
  local Map = workspace:WaitForChild("Map", 9e9)
  task.spawn(function()
    while true do
...
local plrPP = game.Players.LocalPlayer.Character.HumanoidRootPart
        if plrPP then
          Distance = tostring(math.floor((plrPP.Position - Map.FrozenDimension.WorldPivot.Position).Magnitude / 6))
        end
      end
    end
  end)
  ...
  while true do
    task.wait()
    if Map:FindFirstChild("FrozenDimension") then
      FDLabel:SetTitle("Frozen Dimension : Spawned | Distance : " .. Distance)
    else
      FDLabel:SetTitle("Frozen Dimension : not Spawn")
    end
  end
end)
	...
	--[[
	workspace.Boats.Guardian.VehicleSeat
	workspace.Map.PrehistoricIsland.Core.PrehistoricRelic
	workspace.Map.PrehistoricIsland.Core.InteriorLava
	workspace.Map.PrehistoricIsland
	]]--

--—————— Player ——————

--—————— Race ——————
function GetEnemieRace(MonsterName)
for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
if v.Name == MonsterName and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") then
while v.Humanoid.Health > 0 do
if not _G.UpToV2 then
break
end
Tween(v.HumanoidRootPart.Position + Vector3.new(0, getgenv().EnemiePosY, 0))
task.wait()
                ...
                if _G.UpToV2 then
                    if not game:GetService("Players").LocalPlayer.Data.Race:FindFirstChild("Evolved") then
                        if FireRemote("Alchemist", "1") == 0 then
                            Tween(Vector3.new(- 2779, 72, - 3574))
                            if ((Vector3.new(- 2779, 72, - 3574) - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 4) then
                                wait(0.2);
                                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Alchemist", "2");
                            end
                        elseif (game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Alchemist", "1") == 1) then
                            pcall(function()
                                if (not Backpack("Flower 1") and not Character("Flower 1")) then
                                    Tween(game:GetService("Workspace").Flower1.Positio );
                                elseif (not Backpack("Flower 2") and not Character("Flower 2")) then
                                    Tween(game:GetService("Workspace").Flower2.Position);
                                elseif (not Backpack("Flower 3") and not Character("Flower 3")) then
                                        ...
                                        for v1834, v1835 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                            if (v1835.Name == "Zombie") then
                                                repeat
                                                    ...
                                                    AutoHaki();
                                                    EquipTool(SelectWeapon)
                                                    GetEnemieRace("Zombie")
                                                until Backpack("Flower 3") or not v1835.Parent or (v1835.Humanoid.Health <= 0) or (_G.UpToV2 == false)
                                                
                                            end
                                        end
                                    else
                                        Tween(Vector3.new(- 5685, 48, - 853));
                                    end
                                end
                            end)
...
spawn(function()
    while true do
        task.wait()
        if _G.UpToV3 then
            Status = FireRemote("Wenlocktoad", "1")
            if Status == 0 then
                FireRemote("Wenlocktoad", "2")
                wait(0.01)
            elseif Status == -1 then
                Notify.new("Need Race V2"):Display()
                if FireRemote("Wenlocktoad", "2") == 1 and PlayerRace == "Mink" then
                    bausPegos = 0
                    for _, item in pairs(workspace.ChestModels:GetChildren()) do
                        repeat
                            task.wait()
                            Tween(item.WorldPivot.Position)
                        until not item:IsDescendantOf(workspace)
                        bausPegos += 1
                        if bausPegos >= 30 then
...
local prgMoon = Race:AddParagraph({
    ...
    while true do
        task.wait()
        if Sea2 then
        moon = game:GetService("Lighting").FantasySky.MoonTextureId
        end
        if Sea3 then
        ...
        spawn(function()
            while wait() do
                pcall(function()
                    ...
                    if PlayerRace == "Draco" then
                        BlueGearDetails:SetTitle("Blue Gear:  [ V2 100% ]")
                        RedGearDetails:SetTitle("Red Gear:  [ Good ]")
                    end
                end)
                wait(100)
            end
...
spawn(function()
    ...
    while true do
        ...
        if _G.AutoTrainRace then
            if not workspace.Characters[game:GetService("Players").LocalPlayer.Name].RaceTransformed.Value then
                _G.AutoFarmBones = true
                repeat
                    task.wait()
                until not _G.AutoTrainRace or workspace.Characters[game:GetService("Players").LocalPlayer.Name].RaceTransformed.Value
                _G.AutoFarmBones = false
                UnequipTool(SelectWeapon)
            end
            if workspace.Characters[game:GetService("Players").LocalPlayer.Name].RaceTransformed.Value 
            and workspace.Characters[game:GetService("Players").LocalPlayer.Name].RaceEnergy.Value <= 0.4 then
...
Race:AddSection("Only Second and Third Sea")
...
Tween(workspace.Map.PrehistoricIsland.Core.PrehistoricRelic.WorldPivot.Position + Vector3.new(0, 10, 0))